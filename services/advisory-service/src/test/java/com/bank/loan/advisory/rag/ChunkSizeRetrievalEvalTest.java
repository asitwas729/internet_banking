...
class ChunkSizeRetrievalEvalTest {
    ...
    private static final List<Q> QUERIES = List.of(
    ...
    /** 각 질의의 골드 섹션(헤딩 제목 매칭, 본문 200자 초과)을 해석. 실패 질의는 제외. */
    private static Resolved resolveGold(String content) {
        ...
        for (Q q : QUERIES) {
            ...
            // 매칭 섹션 중 본문이 가장 큰 것을 골드로(작은 동명 섹션에 가려지지 않도록)
            Section gold = sections.stream()
                    .filter(s -> norm(s.title()).contains(key) && s.end() - s.start() > 200)
        ...
        if (!unresolved.isEmpty()) {
            System.out.println("[resolveGold] 미해석 질의 골드: " + unresolved);
        }
    ...
    void 하니스_배선_오프라인_검증() throws Exception {
        ...
        Resolved r = resolveGold(content);
    ...
    void 청크크기별_검색품질_Recall_MRR_nDCG_비교() throws Exception {
        ...
        Resolved resolved = resolveGold(content);
    ...
    private static Path locateDoc() {
        ...
        for (int up = 0; up < 6 && dir != null; up++, dir = dir.getParent()) {
            Path candidate = dir.resolve(DOC_RELATIVE);
