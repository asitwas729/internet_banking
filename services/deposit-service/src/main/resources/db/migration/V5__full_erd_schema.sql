...
-- 챗봇 시나리오 (Untitled4)
-- =============================================
CREATE TABLE chatbot_scenario (
    id                          BIGINT,
    chatbot_scenario_name       VARCHAR(100),
    chatbot_scenario_desc       VARCHAR(500),
    chatbot_scenario_type_cd    VARCHAR(10),
    inquiry_type_cd             VARCHAR(10),
    reception_channel_cd        VARCHAR(10),
    test_yn                     CHAR(1),
    active_yn                   CHAR(1),
...
-- =============================================
-- 챗봇 의도 (Untitled3)
-- 자기참조 FK (fallback_intent_id) nullable → 일반 FK
-- =============================================
CREATE TABLE chatbot_intent (
    ...
    updated_by              BIGINT,
    PRIMARY KEY (id),
    CONSTRAINT fk_chatbot_intent_fallback
        FOREIGN KEY (fallback_intent_id) REFERENCES chatbot_intent (id),
    CONSTRAINT fk_chatbot_intent_scenario
        FOREIGN KEY (id2) REFERENCES chatbot_scenario (id)
);
...
CREATE TABLE chatbot_node (
    ...
    updated_by          BIGINT,
    PRIMARY KEY (id),
    CONSTRAINT fk_chatbot_node_scenario
        FOREIGN KEY (id2)              REFERENCES chatbot_scenario (id),
    CONSTRAINT fk_chatbot_node_product
        FOREIGN KEY (linked_product_id) REFERENCES common_product (product_id)
);
...
-- 챗봇 상담 (Untitled2)
-- =============================================
CREATE TABLE chatbot_consultation (
    id                      BIGINT,
    id2                     BIGINT,
    id3                     BIGINT,
    id4                     BIGINT,
    session_id              VARCHAR(20),
    chatbot_process_type_cd VARCHAR(10),
    initial_intent          VARCHAR(100),
    entry_screen            VARCHAR(50),
    app_version             VARCHAR(20),
    session_started_at      TIMESTAMPTZ(3),
    session_ended_at        TIMESTAMPTZ(3),
    total_turn_count        INT,
    resolved_yn             CHAR(1),
    agent_connected_yn      CHAR(1),
    end_type_cd             VARCHAR(10),
    error_occurred_yn       CHAR(1),
    satisfaction_score      SMALLINT,
    chat_transferred_at     TIMESTAMPTZ(3),
    transfer_reason_cd      VARCHAR(20),
    PRIMARY KEY (id),
    CONSTRAINT fk_chatbot_consult_consult
        FOREIGN KEY (id2) REFERENCES consultation (id),
    CONSTRAINT fk_chatbot_consult_scenario
        FOREIGN KEY (id3) REFERENCES chatbot_scenario (id),
    CONSTRAINT fk_chatbot_consult_intent
        FOREIGN KEY (id4) REFERENCES chatbot_intent (id)
);
...
CREATE TABLE chatbot_conversation_history (
    ...
    sentiment_result_cd VARCHAR(10),
    error_type_cd       VARCHAR(10),
    created_at          TIMESTAMPTZ(3),
    created_by          BIGINT,
    updated_at          TIMESTAMPTZ(3),
    updated_by          BIGINT,
    PRIMARY KEY (id2, id),
    CONSTRAINT fk_chatbot_history_consult
        FOREIGN KEY (id2) REFERENCES chatbot_consultation (id),
    CONSTRAINT fk_chatbot_history_node
        FOREIGN KEY (id3) REFERENCES chatbot_node (id)
);
