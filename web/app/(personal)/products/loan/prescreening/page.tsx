              ...
              <>
                ...
                <div className="rounded-xl overflow-hidden mb-5" style={{ border: '1px solid #E2F5EF' }}>
                  <div className="px-5 py-3 text-[13px] font-semibold"
                    style={{ backgroundColor: '#F0FAF7', borderBottom: '1px solid #E2F5EF', color: '#0D5C47' }}>
                    가심사 대상 신청 선택
                  </div>
                  {applications.length === 0 ? (
                    <div className="px-5 py-10 text-center">
                      ...
                      <Link href="/products/loan/credit"
                        ...
                        style={{ backgroundColor: '#0D5C47' }}>대출 신청하기</Link>
                    </div>
                  ) : (
                        ...
                        {applications.map((app, i, arr) => (
                          <tr key={app.applId}
                            ...
                            style={{
                              ...
                              backgroundColor: selected?.applId === app.applId ? '#F0FAF7' : undefined,
                            }}>
                            <td className="px-4 py-3 text-center">
                              <input type="radio" checked={selected?.applId === app.applId}
                                onChange={() => setSelected(app)} style={{ accentColor: '#0D5C47' }} />
                            </td>
                            <td className="px-4 py-3 text-center font-medium" style={{ color: '#0D5C47' }}>{app.applNo}</td>
                ...
                {result && (
                  <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #E2F5EF' }}>
                    ...
                    <div className="grid grid-cols-3" style={{ borderBottom: '1px solid #E2F5EF' }}>
                      {[
                        { label: '심사 결과',    value: result.prescreeningResultCd === 'PASS' ? '통과' : '거절',
                          color: result.prescreeningResultCd === 'PASS' ? '#0D5C47' : '#E05555' },
                        { label: '예상 한도',    value: `${(result.estimatedLimit ?? 0).toLocaleString('ko-KR')}원`, color: '#0D5C47' },
                        { label: '예상 금리',    value: `연 ${((result.estimatedRate ?? 0) / 100).toFixed(2)}%`, color: '#0D5C47' },
                    ...
                    <div className="px-5 py-3 text-[12px] text-kb-text-muted" style={{ backgroundColor: '#F8FFFE' }}>
