...
const STEPS = ['상품 선택', '정보 입력', '본인확인', '결과 확인']
          ...
          <div key={label} className="flex items-center">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[13px] font-bold
                ${i <= 1 ? 'bg-[#0D5C47] text-white' : 'border border-[#E2F5EF] text-kb-text-muted bg-white'}`}>
                {i <= 1 ? '✓'.charAt(0) : i + 1}
              </div>
              <span className={`text-[11px] mt-1 font-medium whitespace-nowrap ${i <= 1 ? 'text-[#0D5C47] font-bold' : 'text-kb-text-muted'}`}>{label}</span>
            </div>
            {i < STEPS.length - 1 && <div className={`w-12 h-px mx-2 mb-4 ${i <= 0 ? 'bg-[#0D5C47]' : 'bg-kb-border'}`} />}
          </div>
        ))}
      </div>
            ...
            {products.map(p => (
              <button key={p.prodId} onClick={() => setSelectedProdId(p.prodId)}
                className={`border rounded-xl p-6 text-left transition-colors ${
                  selectedProdId === p.prodId ? 'border-[#0D5C47] bg-[#0D5C47]/20' : 'border-[#E2F5EF] hover:bg-[#F0FAF7]'}`}>
              ...
              {PERIODS.map(p => (
                <button key={p} onClick={() => setPeriod(p)}
                  className={`border px-4 py-1.5 text-[12px] rounded-lg transition-colors ${
                    period === p ? 'bg-[#0D5C47] border-[#0D5C47] font-bold text-white' : 'border-[#E2F5EF] text-kb-text-muted hover:bg-[#F0FAF7]'}`}>
                  {p}개월
                </button>
              ))}
            </div>
      ...
      <section className="mb-6">
        ...
        <div className="flex justify-start">
          <button onClick={handlePreview} disabled={!canPreview}
            className={`px-8 py-2 text-[13px] font-medium border transition-colors ${
              canPreview ? 'border-[#0D5C47] text-kb-text hover:bg-[#F0FAF7]' : 'border-[#E2F5EF] text-kb-text-muted cursor-not-allowed'}`}>
        ...
        {previewResult && (
          <div className="mt-4 border border-[#E2F5EF]">
            ...
            <div className="divide-y divide-[#E2F5EF] text-[13px]">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <button type="button" onClick={() => setAgreed(v => !v)}
              className={`w-4 h-4 border flex-shrink-0 flex items-center justify-center transition-colors
                ${agreed ? 'bg-[#0D5C47] border-[#0D5C47]' : 'bg-white border-[#E2F5EF]'}`}>
