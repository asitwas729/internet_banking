...
export default function BizMyMenu() {
  const pathname = usePathname()
  const [open, setOpen] = useState(true)
  const [zoomIndex, setZoomIndex] = useState(DEFAULT_INDEX)

  if (pathname === '/biz/login') return null
  ...
  return (
    <div className="fixed right-0 top-1/3 z-40 flex items-start">
      ...
      <button
        ...
        className="mt-14 w-6 h-16 bg-kb-yellow flex items-center justify-center
                   border border-kb-border border-r-0 rounded-l-lg shadow-sm hover:bg-kb-yellow-dark transition-colors"
        style={{ writingMode: 'vertical-rl' }}
      >
        <span className="text-[10px] font-bold text-kb-text rotate-180">
          {open ? '◀' : '▶'}
        </span>
      </button>
        ...
        <div className="bg-white border border-kb-border shadow-xl overflow-hidden w-[320px] text-sm rounded-l-2xl">
          ...
          <div className="px-5 pt-6 pb-4">
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1">
              {['계좌조회', '계좌이체', '외환HOME', '증명서/확인증 발급'].map((item) => (
                <Link key={item} href="#"
